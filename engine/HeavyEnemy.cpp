...
short HeavyEnemy::getTypeID() {
    return 2;
}

HeavyEnemy::HeavyEnemy(const Context &c) {
...
void HeavyEnemy::draw(sf::RenderWindow &window) {
    window.draw(shape);
}

void HeavyEnemy::update(Context &context) {
...
void HeavyEnemy::takeDamage(unsigned short amount, Context &context) {
    health -= amount;
    if (health <= 0) {
